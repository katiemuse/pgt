...
(function(a){SfdcWwwBase.oneTrustComponent.changeButtons=function(){var e=a(".ot-sdk-container").find(".save-preference-btn-handler");
var d=a(".ot-sdk-container").find("#accept-recommended-btn-handler");
var c=a(".category-menu-switch-handler");
function b(f){if(typeof a(f).closest("li").data("optanongroupid")=="undefined"){e.removeClass("visible");
...
}}c.click(function(f){b(f.currentTarget)
});
c.keydown(function(f){if(f.keyCode===32){b(f.currentTarget)
}});
d.keydown(function(f){if(f.key==="Enter"||f.key===" "){a("#onetrust-consent-sdk").hide();
a(".optanon-toggle-display").focus()
}});
e.keydown(function(f){if(f.key==="Enter"||f.key===" "){a("#onetrust-consent-sdk").hide();
a(".optanon-toggle-display").focus()
}})
};
...
SfdcWwwBase.oneTrustComponent.overrideSettings=function(){var b=a("#onetrust-pc-sdk #vendors-list").find("#accept-recommended-btn-handler");
b.addClass("optanon-ghost-button");
a(".menu-item-about, .menu-item-necessary, .menu-item-functional, .menu-item-advertising, .optanon-white-button-middle").mousedown(function(c){c.preventDefault()
});
a(".optanon-toggle-display").click(function(){a("#onetrust-consent-sdk").show();
a(".active-group").focus();
a(".optanon-alert-box-wrapper").hide()
});
a(".optanon-toggle-display").keydown(function(c){if(c.key==="Enter"||c.key===" "){a(".optanon-toggle-display").click()
}});
